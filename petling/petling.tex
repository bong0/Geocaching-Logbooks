...
\usepackage[left=0.5cm,right=0.5cm,bottom=0.9cm,head=1cm,foot=0cm]{geometry}
\geometry{a4paper}
\input{../common/header}
		...
		\forloop[1]{col}{1}{\value{col} < 3}{	 	
			\begin{minipage}{\linewidth} %encapsulate text to prevent moving to other column
				...
				\begin{Huge}
					...
					\begin{align*}
						\begin{pspicture}
		   						\psbarcode[scalex=0.8, scaley=0.8]{\gccBaseURL\gccID}{}{qrcode}
		  				\end{pspicture}
						&&\makebox[4.5cm][c]{\fillrule{0.1mm}}&&
						\begin{pspicture}
				   			\psbarcode[scalex=0.8, scaley=0.8]{\ocBaseURL\ocID}{}{qrcode}
				  		\end{pspicture}
					\end{align*}

					%\vspace*{0.3cm}
					\includegraphics[width=0.45\linewidth]{../common/Geocaching_Logo} \\
					\vspace*{-0.1cm}
